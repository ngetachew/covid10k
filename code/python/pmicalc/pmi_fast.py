...
def generate_co_occurrence_matrix(v):
        co_occ = dok_matrix((v.shape[0],v.shape[0]), dtype=np.int32)
        count = len(files)
        ...
        for f in files[:100]:
                ...
                progress += 1
                print('done')
        np.savez_compressed('co_occ_matrix_10KQ_12-1',co_occurrence=co_occ)
...
df_pmi.head(1000).to_csv('pmi10K_fast.csv')
