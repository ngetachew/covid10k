...
if [ ! -f $outputprefix.gz ]; then
    ...
    mallet train-topics --input $seqfile --num-topics $k --optimize-interval 10 --num-iterations $iterations --output-state $outputprefix.gz --output-topic-keys $outputprefix.keys.txt --num-top-words 50 --diagnostics-file $outputprefix.diag.xml --evaluator-filename $outputprefix.evaluator --word-topic-counts-file $outputprefix.wordcounts.txt --topic-word-weights-file $outputprefix.wordweights.txt --output-doc-topics $outputprefix.doctopics.txt &> $outputprefix.out
fi
...
for i in {1..9}; do
    ...
    if [ -f $prevoutputprefix.gz ]; then
        ...
        iterations=100
        echo train $outputprefix $seqfile
        mallet train-topics --input $seqfile --num-topics $k --optimize-interval 10 --num-iterations $iterations --input-state $prevoutputprefix.gz --output-state $outputprefix.gz --output-topic-keys $outputprefix.keys.txt --num-top-words 50 --diagnostics-file $outputprefix.diag.xml --evaluator-filename $outputprefix.evaluator --word-topic-counts-file $outputprefix.wordcounts.txt --topic-word-weights-file $outputprefix.wordweights.txt --output-doc-topics $outputprefix.doctopics.txt &> $outputprefix.out
    fi
done
